...
export function ShippingQuote({
  ...
  const isFreeEffective = isFree || subtotal < 30;
      ...
      if (finalData.rates && finalData.rates.length > 0) {
        const validOptions: ShippingOption[] = finalData.rates
          ...
          .map((opt: any) => ({
            ...
            price: isFreeEffective ? 0 : parseFloat(opt.amount),
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-black text-[#00AAC7]">
                      {isFreeEffective ? "GRATIS" : `$${option.price.toLocaleString()}`}
                    </p>
                    <p className="text-xs text-gray-500">{isFreeEffective ? "Prueba" : "MXN"}</p>
                  </div>
                </div>
              </motion.div>
            ))}
