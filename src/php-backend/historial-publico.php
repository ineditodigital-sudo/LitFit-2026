...
/**
 * ============================================
 * ADMIN API - DETALLE TOTAL (V47)
 * ============================================
 * Trae remitente, destinatario y paquetes para el Modal.
 */
...
curl_setopt($chAuth, CURLOPT_POSTFIELDS, json_encode([
...
function fetchFullCollection($base, $tok) {
    $allData = [];
    $allIncluded = [];
    $page = 1;
    
    do {
        // 💡 CLAVE: Incluimos remitente (address_from) para el "Ojo"
        $url = "$base/shipments?include=address_to,address_from,packages&page=$page";
        $ch = curl_init($url);
...
echo json_encode(fetchFullCollection($base, $tok));
exit;
