...
// ============================================
// FUNCIÓN PARA OBTENER TOKEN FRESCO - CORREGIDA
// ============================================
function obtenerTokenFresco() {
    error_log("🔐 Intentando autenticación con Client ID: " . substr(ENVIOS_CLIENT_ID, 0, 10) . "...");
    
    // MÉTODO 1: Form URL Encoded (Estándar OAuth2)
    ...
    // MÉTODO 2: JSON (Fallback por si la API acepta JSON)
    error_log("⚠️ Método 1 falló, intentando con JSON...");
    ...
    error_log("📥 Respuesta autenticación JSON HTTP: " . $httpCode2);
    error_log("📥 Respuesta JSON body: " . substr($response2, 0, 200));

    if ($httpCode2 === 200) {
        $json2 = json_decode($response2, true);
        if (isset($json2['access_token'])) {
            error_log("✅ Token obtenido con método JSON");
            return $json2['access_token'];
        }
    }
    
    // MÉTODO 3: Basic Authentication (Otro estándar común)
    error_log("⚠️ Método 2 falló, intentando con Basic Auth...");
    ...
    error_log("📥 Respuesta autenticación Basic Auth HTTP: " . $httpCode3);
    error_log("📥 Respuesta Basic Auth body: " . substr($response3, 0, 200));

    if ($httpCode3 === 200) {
        $json3 = json_decode($response3, true);
        if (isset($json3['access_token'])) {
            error_log("✅ Token obtenido con Basic Auth");
            return $json3['access_token'];
        }
    }
    
    error_log("❌ Error obteniendo Token después de 3 intentos");
    error_log("❌ Último HTTP Code: " . $httpCode3 . " | Respuesta: " . $response3);
    return false;
}
...
error_log("📋 Payload: " . json_encode($shipmentData));
