 ...
 * 1. Obtener token de autenticación (Vía x-www-form-urlencoded)
 * 2. Crear un envío de prueba con ese token (Endpoint /rate/shipments/)
 */
...
// ⚠️ CORRECCIÓN CRÍTICA: Endpoint para crear envío y generar guía
define('CREATE_ORDER_URL', 'https://app.enviosinternacionales.com/api/v1/rate/shipments/');
...
<head>
    ...
    <style>
        body { font-family: 'Courier New', monospace; background: #1a1a1a; color: #00ff00; padding: 20px; margin: 0; }
        .container { max-width: 1200px; margin: 0 auto; background: #0a0a0a; padding: 30px; border: 2px solid #00AAC7; border-radius: 10px; }
        h1 { color: #00AAC7; text-align: center; font-size: 28px; margin-bottom: 30px; text-transform: uppercase; }
        .test-section { background: #1a1a1a; border: 1px solid #333; padding: 20px; margin: 20px 0; border-radius: 5px; }
        .test-title { color: #FFD700; font-size: 18px; font-weight: bold; margin-bottom: 15px; border-bottom: 2px solid #333; padding-bottom: 10px; }
        .success { color: #00ff00; font-weight: bold; }
        .error { color: #ff4444; font-weight: bold; }
        .warning { color: #ffaa00; font-weight: bold; }
        .info { color: #00AAC7; }
        pre { background: #000; padding: 15px; border-left: 4px solid #00AAC7; overflow-x: auto; font-size: 12px; line-height: 1.5; max-height: 400px; overflow-y: auto; }
    </style>
</head>
<body>
    <div class="container">
...
if ($httpCode1 == 200 && isset($json1['access_token'])) {
    ...
    echo '<p class="error">❌ NO se pudo obtener el token. Revisa tus llaves. Proceso detenido.</p>';
...
// ⚠️ CORRECCIÓN CRÍTICA: Estructura exacta para el endpoint /rate/shipments/
$shipmentData = [
    'carrier' => [
        'name' => 'fedex', // Asegúrate de que este carrier esté activo en tu cuenta
        'service_name' => 'standard_overnight' // O el servicio equivalente en México
    ],
    'address_from' => [
        'country_code' => 'MX',
        'postal_code' => '64000',
    ...
    'address_to' => [
        ...
        'area_level3' => 'Vistas de Oriente',
        'company' => 'Particular',
        'name' => 'Ricardo Ledesma',
        'phone' => '4492610335',
        'email' => 'ricoro845@gmail.com',
        'street1' => 'Vallarta 216 #8',
...
echo '<p><span class="info">📤 Datos del envío (Endpoint: /rate/shipments/):</span></p>';
...
// ============================================
// INTENTO 1: Authorization: Bearer {token} (ESTE ES EL ESTÁNDAR)
// ============================================
...
echo '<p><span class="info">📥 HTTP Status Code:</span> <span class="' . ($httpCode2 == 200 || $httpCode2 == 201 || $httpCode2 == 202 ? 'success' : 'error') . '">' . $httpCode2 . '</span></p>';
...
// Mostrar análisis
if (empty($response2)) {
    echo '<p class="warning">⚠️ La API devolvió una respuesta vacía.</p>';
} else {
    $responseJson = json_decode($response2, true);
    if ($responseJson) {
        echo '<p class="info">📋 JSON Parseado:</p>';
...
echo '<p><span class="info">📥 HTTP Status Code:</span> <span class="' . ($httpCode3 == 200 || $httpCode3 == 201 || $httpCode3 == 202 ? 'success' : 'error') . '">' . $httpCode3 . '</span></p>';
echo '<p><span class="info">📥 Response:</span></p>';
...
echo '<p><span class="info">📥 HTTP Status Code:</span> <span class="' . ($httpCode4 == 200 || $httpCode4 == 201 || $httpCode4 == 202 ? 'success' : 'error') . '">' . $httpCode4 . '</span></p>';
echo '<p><span class="info">📥 Response:</span></p>';
echo '<pre>' . htmlspecialchars(substr($response4, 0, 500)) . '</pre>';
echo '</div>';
...
$metodoExitoso = null;
$exitoCodes = [200, 201, 202]; // La API de envíos a veces devuelve 201 (Created) o 202 (Accepted)

if (in_array($httpCode2, $exitoCodes)) $metodoExitoso = '✅ INTENTO 1: Authorization: Bearer {token}';
if (in_array($httpCode3, $exitoCodes)) $metodoExitoso = '✅ INTENTO 2: Token en URL (?access_token=...)';
if (in_array($httpCode4, $exitoCodes)) $metodoExitoso = '✅ INTENTO 3: X-API-TOKEN header';
...
if ($metodoExitoso) {
    ...
    echo '<p style="color: #ffaa00;">📋 ACCIÓN: Ve al panel de EnviosInternacionales y confirma que se generó la guía de prueba con destino a Aguascalientes.</p>';
} else {
    echo '<p class="error" style="font-size: 20px;">❌ NINGÚN MÉTODO FUNCIONÓ</p>';
    echo '<p class="warning">⚠️ Revisa el JSON Parseado en el Intento 1. La API suele decir exactamente qué campo le molesta (por ejemplo, código postal inválido, carrier no activo, etc).</p>';
}
